...
class ArgsToAcquisitionParametersDataFrame:
    ...
    def get_exp_data_parameters_directory(self):
        fpm = FilePathManager()
    ...
    def _get_setup_notes(self, element):
        path_exp_data_params = self.get_exp_data_parameters_directory()
    ...
    def _get_list_of_files_paths(self, element, date):
        ...
        main_directory_for_exp_files = path_manager.get_directory_for_exp_data(
            element, date
        )
...
class AcquisitionParametersFinder(ArgsToAcquisitionParametersDataFrame):
    def __init__(self, element, path):
        ...
        self.files_info_df = self.make_df(path)
        # self.assign_experimental_parameters()
    ...
    def assign_experimental_parameters(self):
        ...
        self.files_info_df = self._assign_discharge_nrs(
            self.files_info_df, self.triggers_df, self.date
        )
        self.files_info_df = self._shift_to_T1(self.files_info_df, self.triggers_df)
    ...
    def make_df(self, path) -> pd.DataFrame:
        fname_list = self.get_file_names(path)
...
for dir_path in list_of_directories:
    # ArgsToAcquisitionParametersDataFrame(element, dir_path)
    AcquisitionParametersFinder(element, dir_path).assign_experimental_parameters()
