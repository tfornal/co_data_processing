...
def get_exp_data_parameters_directory():
    fpm = FilePathManager()
...
def _get_frequency(
    files_info_df, element, date, camera_setups, file_list, pulse_lengths
):
...
def _assign_discharge_nrs(files_info_df, triggers_df, date):
    ...
    if dictionary:
        files_info_df["discharge_nr"].loc[np.array(list(dictionary.keys()))] = np.array(
            list(dictionary.values())
        )
        indexes_to_assign = files_info_df.index[~files_info_df.index.isin(dictionary)]
...
class Arguments:
    def __init__(self, element):
        ...
        self.path = self._get_exp_data_subdirs(self.element)[0]
        ...
        self.pulse_lengths = self._get_pulse_length(self.element, self.date)
        self.camera_setups = self._get_setup_notes(self.element)

    def _get_exp_data_subdirs(self, element):
        """
    ...
    def _get_pulse_length(self, element, date):
        path_manager = FilePathManager()
        path = path_manager.get_directory_for_exp_data(element, date)
        ...
        return files_list, pulse_lengths


Arguments("C")


class AcquisitionParametersFinder(Arguments):
    def __init__(self):
        super.__init__(element)
        self.files_info_df = make_df(path)

    def assign_experimental_parameters(self):
        # files_info_df = make_df(path)
        _get_utc_time(self.files_info_df)
        files_info_df = _get_frequency(
            self.files_info_df, element, date, camera_setups, file_list, pulse_lengths
        )
        _update_acquisition_time(self.files_info_df)
        _calc_utc_start_time_ns(self.files_info_df)
        _assign_discharge_nrs(self.files_info_df, triggers_df, date)
        _shift_to_T1(self.files_info_df, triggers_df)


# for i in files_info.columns:
#     print(files_info[f"{i}"])
# # if savefile:
# # save_file()
